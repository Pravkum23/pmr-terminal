...
def _text(s, x, y, w, h, txt, size=12, color=TXT, bold=False):
...
def build_deck(d: dict, path: str = "docs/pmr_daily.pptx"):
    ...
    cur = "Rs " if p["base_currency"] == "INR" else "$"

    # Slide 1: cover / summary
    s = _slide(prs)
    _text(s, 0.6, 1.7, 12, 1.0, "PMR TERMINAL", 48, CYAN, True)
    _text(s, 0.62, 2.7, 12, 0.5, "Daily Market Intelligence · by Praveen", 16, TXT)
    ...
    for col, (label, items) in enumerate((("GAINERS", m["gainers"]), ("LOSERS", m["losers"]))):
        ...
        for i, r in enumerate(items):
            ...
            _text(s, x0, y + 0.33, 3.2, 0.3, f"{r['symbol']} · {r['rag']}", 9, RAGC[r["rag"]])
